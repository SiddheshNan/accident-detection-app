...
import { API_URL, showAlert, APP_SESSION } from "../../utils";
...
export default function ResponderLogin({ navigation }) {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      axios
        .post(`${API_URL}/responder/login`, {
          username: myN,
          password: myP,
        })
        .then(async (response) => {
          if (response?.data?.msg === "ok") {
            showAlert("Success", "Responder Logged In Successfully!");
            APP_SESSION.username = myN;
            APP_SESSION._id = response.data.responder._id;
